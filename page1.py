...
#     """
#     Calcule le prix optimal pour une quantité donnée en utilisant les prix connus.
#     Ignore les prix à -1 (non disponibles).
    
#     Args:
#         prices_dict: Dictionnaire {quantité_str: prix}
#         target_quantity: Quantité cible (int)
    
#     Returns:
#         Prix calculé (int) ou None si impossible
#     """
#     if not prices_dict:
#         return None
    
#     # Convertir les clés en int et filtrer les None et les -1
...
#                 if price_float > 0:  # On s'assure que le prix est positif
#                     available_quantities[qty_int] = price_float
...
#     sorted_quantities = sorted(available_quantities.keys(), reverse=True)
    
#     # Si la quantité cible est plus petite que la plus petite disponible
#     # On divise le prix de la plus petite quantité disponible
#     if target_quantity < sorted_quantities[-1]:
#         smallest_qty = sorted_quantities[-1]
#         result = available_quantities[smallest_qty] * target_quantity / smallest_qty
#         return int(round(result))
    
#     # Algorithme glouton pour trouver la meilleure combinaison
...
#         # Nombre de fois qu'on peut utiliser cette quantité
#         times = remaining // qty
#         if times > 0:
#             total_cost += times * available_quantities[qty]
#             remaining -= times * qty
    
#     # S'il reste un résidu, utiliser la plus petite quantité disponible
#     if remaining > 0:
#         smallest_qty = sorted_quantities[-1]
#         total_cost += available_quantities[smallest_qty] * remaining
...
#     craft_val = calculate_optimal_price(craft, quantity)
...
#     # Contrôle de la quantité avec le même style que la pagination (avec - et +)
...
#     # Calculer les valeurs avec la nouvelle fonction
#     prix_val = calculate_optimal_price(prix_dict, quantity)
#     craft_val = calculate_optimal_price(craft_dict, quantity)
...
# def calculate_optimal_price(prices_dict, target_quantity):
#     """
#     Calcule le prix optimal pour une quantité donnée en utilisant la programmation dynamique.
#     Ignore les prix à -1 (non disponibles).
...
#     # Si la quantité cible est plus petite que la plus petite disponible
#     # On divise le prix de la plus petite quantité disponible
#     if target_quantity < sorted_quantities[0]:
#         smallest_qty = sorted_quantities[0]
#         result = available_quantities[smallest_qty] * target_quantity / smallest_qty
#         return int(round(result))
    
#     # Programmation dynamique pour trouver le prix optimal
#     # dp[i] = coût minimal pour obtenir exactement i unités
#     dp = [float('inf')] * (target_quantity + 1)
#     dp[0] = 0
    
#     for i in range(1, target_quantity + 1):
#         # Pour chaque quantité i, on teste toutes les quantités disponibles
#         for qty, price in available_quantities.items():
#             if qty <= i:
#                 # On peut utiliser cette quantité
#                 dp[i] = min(dp[i], dp[i - qty] + price)
#             else:
#                 # La quantité disponible est plus grande que ce qu'on veut
#                 # On peut en acheter une fraction (en payant le prix complet)
#                 # Mais seulement si c'est avantageux
#                 fractional_price = price * i / qty
#                 dp[i] = min(dp[i], fractional_price)
...
def calculate_optimal_price(prices_dict, target_quantity):
    ...
    Calcule le prix optimal pour une quantité donnée en utilisant la programmation dynamique.
    Ignore les prix à -1 (non disponibles).
    Si on achète une quantité supérieure, on paie le prix complet (pas de division).
    ...
    # Programmation dynamique pour trouver le prix optimal
    # dp[i] = coût minimal pour obtenir au moins i unités
    dp = [float('inf')] * (target_quantity + 1)
    dp[0] = 0
    ...
    for i in range(1, target_quantity + 1):
        ...
        for qty, price in available_quantities.items():
            if qty <= i:
                # On utilise cette quantité et on complète avec dp[i - qty]
                dp[i] = min(dp[i], dp[i - qty] + price)
        
        # Option 2: Acheter directement une quantité supérieure à i
        # (on paie le prix complet même si on n'utilise pas tout)
        for qty, price in available_quantities.items():
            if qty > i:
                # On achète cette quantité (qui couvre i et plus)
                dp[i] = min(dp[i], price)
